...
func (eb *EthBroadcaster) processUnstartedEthTxs(ctx context.Context, fromAddress gethCommon.Address) (err error, retryable bool) {
	...
	oldestUnconfirmedTx, err := findOldestUnconfirmedTransactionFromAddress(eb.db, fromAddress, eb.chainID)
	if err != nil {
		return errors.Wrap(err, "findOldestUnconfirmedTransactionFromAddress failed"), true
	}
	if oldestUnconfirmedTx != nil {
		if time.Since(oldestUnconfirmedTx.CreatedAt) > eb.config.EthTxLogUnconfirmedAfterThreshold() {
...
func findOldestUnconfirmedTransactionFromAddress(db *sqlx.DB, fromAddress gethCommon.Address, chainID big.Int) (*EthTx, error) {
	etx := new(EthTx)
	err := db.Get(&etx, `SELECT * FROM eth_txes WHERE from_address = $1 AND state = 'unconfirmed' AND evm_chain_id = $2 ORDER BY created_at ASC, value ASC, id ASC`, fromAddress, chainID.String())
